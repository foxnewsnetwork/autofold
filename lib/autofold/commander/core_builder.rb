module Autofold
  class Commander
    class CoreBuilder < Thor
      attr_accessor :destination, :absolute_path, :project_name, :class_project_name,
      ...
      def build(path, parent, child)
        ...
        _directory _base_root, absolute_path
      end

      private

      # Recursively copies files, injects into if file exists, else creates it
      def _directory(source, destination)
      ...
      def _base_root
        File.join self.class.source_root
      end
